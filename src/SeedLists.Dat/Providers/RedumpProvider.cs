...
/// <summary>
/// Redump provider using local DAT/archives from operator-managed directories.
/// </summary>
public sealed class RedumpProvider(IOptions<SeedListsDatOptions> options) : IDatProvider {
	...
	public Task<Stream> DownloadDatAsync(string identifier, CancellationToken cancellationToken = default) {
		...
		if (!identifier.StartsWith("local::", StringComparison.OrdinalIgnoreCase)) {
			throw new NotSupportedException("Redump provider currently supports only local identifiers.");
		}
