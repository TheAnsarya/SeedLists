...
/// </summary>
public sealed class MameProvider(IOptions<SeedListsDatOptions> options) : IDatProvider {
	...
	public Task<Stream> DownloadDatAsync(string identifier, CancellationToken cancellationToken = default) {
		...
		if (!identifier.StartsWith("local::", StringComparison.OrdinalIgnoreCase)) {
			throw new NotSupportedException("MAME provider currently supports only local identifiers.");
		}
